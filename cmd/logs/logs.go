...
var Logs = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !exist {
			...
			if QuayString == "" {
				fmt.Fprintln(cmd.ErrOrStderr(), "Some error occurred, wrong must-gather file composition")
				os.Exit(1)
			}
		}
		...
		if len(args) == 0 || len(args) > 2 {
			fmt.Fprintln(cmd.ErrOrStderr(), "error: expected 'logs [-p] (POD | TYPE/NAME) [-c CONTAINER]'.")
			fmt.Fprintln(cmd.ErrOrStderr(), "POD or TYPE/NAME is a required argument for the logs command")
			fmt.Fprintln(cmd.ErrOrStderr(), "See 'omc logs -h' for help and examples")
			os.Exit(1)
		}
		if len(args) == 1 {
			if s := strings.Split(args[0], "/"); len(s) == 2 && (s[0] == "po" || s[0] == "pod" || s[0] == "pods") {
				podName = s[1]
				if podName == "" {
					fmt.Fprintln(cmd.ErrOrStderr(), "arguments in resource/name form must have a single resource and name")
					os.Exit(1)
				}
				logsPods(cmd, vars.MustGatherRootPath, vars.Namespace, podName, containerName, previousFlag, rotatedFlag, allContainersFlag, logLevels, insecureFlag)
			} else {
				podName = s[0]
				logsPods(cmd, vars.MustGatherRootPath, vars.Namespace, podName, containerName, previousFlag, rotatedFlag, allContainersFlag, logLevels, insecureFlag)
			}
		...
		if len(args) == 2 {
			if s := strings.Split(args[0], "/"); len(s) == 2 && (s[0] == "po" || s[0] == "pod" || s[0] == "pods") {
				if containerName != "" {
					fmt.Fprintln(cmd.ErrOrStderr(), "error: only one of -c or an inline [CONTAINER] arg is allowed")
					os.Exit(1)
				} else {
					podName = s[1]
					if podName == "" {
						fmt.Fprintln(cmd.ErrOrStderr(), "error: arguments in resource/name form must have a single resource and name")
						os.Exit(1)
					}
